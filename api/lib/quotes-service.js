...
async function fetchFmpJson(url, fetchImpl) {
...
async function fetchFmpAnalystData(symbol, fmpKey, fetchImpl) {
  if (!fmpKey) {
  ...
  for (const u of targetCandidates) {
    const json = await fetchFmpJson(u, fetchImpl);
    if (!json) continue;
...
export async function getQuotesResponse({ symbolsRaw, finnhubKey, fmpKey, fetchImpl = fetch }) {
  if (!finnhubKey) {
    return {
      status: 500,
      payload: {
        error: "missing_finnhub_api_key",
        message: "Set FINNHUB_API_KEY in environment variables.",
      },
    };
  }
  ...
  if (!symbols.length) {
    return { status: 400, payload: { error: "missing_symbols" } };
  }
  ...
  for (const symbol of symbols) {
    ...
    const quoteResp = await fetchImpl(quoteUrl);
    ...
    if (!analystData && fmpKey) {
      try {
        analystData = await fetchFmpAnalystData(providerSymbol, fmpKey, fetchImpl);
  ...
  return {
    status: 200,
    payload: { source: fmpKey ? "Finnhub+FMP" : "Finnhub", quotes, missing },
  };
}
